...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid THRILL address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    if (CoinControlDialog::coinControl->HasSelected()) {
        ...
        //ui->frameCoinControl->setStyleSheet("#SendCoinsDialog #scrollArea {margin-top:-50px} margin-bottom:-50px;");
