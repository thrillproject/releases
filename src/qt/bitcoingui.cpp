                                                                            ...
                                                                            spinnerFrame(0)
{
    ...
    GUIUtil::restoreWindowGeometry("nWindow", QSize(1001, 650), this);

    QString windowTitle = tr("Thrill-Coin") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions(const NetworkStyle* networkStyle)
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Thrill address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and thrill: URIs)"));
	...
	///	Lock
    lockWalletAction2 = new QAction(QIcon(":/icons/lock_closed"),tr("&Lock Wallet..."), this);
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Thrill Core"), this);
    aboutAction->setStatusTip(tr("Show information about Thrill Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Thrill"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Thrill addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Thrill addresses"));
    ...
    oneLogo = new QAction(QIcon(":/icons/logo"), tr("Official site link"), this);
    oneLogo->setStatusTip(tr("Official site"));
    oneLogo->setToolTip(oneLogo->statusTip());

    twoLogo = new QAction(QIcon(":/icons/discord"), tr("Discord"), this);
    twoLogo->setStatusTip(tr("Discord"));
    twoLogo->setToolTip(twoLogo->statusTip());

    threeLogo = new QAction(QIcon(":/icons/telegram"), tr("Telegram"), this);
    threeLogo->setStatusTip(tr("Telegram"));
    threeLogo->setToolTip(threeLogo->statusTip());

    fourLogo = new QAction(QIcon(":/icons/twitter"), tr("Twitter"), this);
    fourLogo->setStatusTip(tr("Twitter"));
    fourLogo->setToolTip(fourLogo->statusTip());
    ...
    openAction->setStatusTip(tr("Open a Thrill: URI or payment request"));
    openBlockExplorerAction = new QAction(QIcon(":/icons/explorer"), tr("&Blockchain explorer"), this);
    openBlockExplorerAction->setStatusTip(tr("Block explorer window"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Thrill Core help message to get a list with possible Thrill command-line options"));
    ...
    connect(oneLogo, SIGNAL(triggered()), this, SLOT(oneLogourl()));
    connect(twoLogo, SIGNAL(triggered()), this, SLOT(twoLogourl()));
    connect(threeLogo, SIGNAL(triggered()), this, SLOT(threeLogourl()));
    connect(fourLogo, SIGNAL(triggered()), this, SLOT(fourLogourl()));
...
void BitcoinGUI::createToolBars()
{
    if (walletFrame) {
        //1-e menu;
        QToolBar* firstmenu = new QToolBar(tr("First menu"));
        firstmenu->setFixedWidth(100);
        firstmenu->setStyleSheet(" QToolBar { background-color: #02a1e5; max-width:150px; min-width: 150px; min-height: 100px; max-height: 100px; } QToolBar > QToolButton { background-color: #02a1e5; min-height:100px; max-height:100px; min-width:100px; max-width:100px; padding-left:25px; }");
        firstmenu->setToolButtonStyle(Qt::ToolButtonIconOnly);
        firstmenu->setIconSize(QSize(100, 100)); // Общий размер иконок

        firstmenu->addAction(oneLogo);

        //2-e menu;
        QToolBar* secondmenu = new QToolBar(tr("Tabs toolbar"));

        secondmenu->setStyleSheet("QToolBar { background-color: #02a1e5; min-width:700px; min-height: 100px; max-height: 100px;} QToolBar > QToolButton { background-color: #02a1e5; min-width: 100px; max-width: 100px; min-height: 90px; max-height: 90px; padding-top: 10px; font-size:12px; } QToolBar > QToolButton:checked { border-image: url(:/icons/active_menu) 0 0 0 0 stretch stretch; border:0px; } ");
        secondmenu->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
        secondmenu->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Preferred);
        secondmenu->setIconSize(QSize(70, 70)); // Общий размер иконок

        secondmenu->addAction(overviewAction);
        secondmenu->addAction(sendCoinsAction);
        secondmenu->addAction(receiveCoinsAction);
        secondmenu->addAction(historyAction);
        QSettings settings;
        if (settings.value("fShowMasternodesTab").toBool()) {
            secondmenu->addAction(masternodeAction);
        }


        //secondmenu->setMovable(false); // remove unused icon in upper left corner
        overviewAction->setChecked(true);

        QToolBar* fourmenu = new QToolBar(tr("Lock Menu"));

        fourmenu->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
        fourmenu->setIconSize(QSize(70, 70));

        fourmenu->addAction(unlockWalletAction);
        fourmenu->addAction(lockWalletAction2);

        fourmenu->setStyleSheet("QToolBar { background-color: #02a1e5; min-width:130px; max-width:130px; min-height: 90px; max-height: 90px; } QToolBar > QToolButton { background-color: #02a1e5;  min-height: 90px; max-height: 90px; font-size:11px;} ");

        //3-e menu;
        QToolBar* threemenu = new QToolBar(tr("Three menu"));
        threemenu->setFixedWidth(170);

        threemenu->setStyleSheet(" QToolBar { background-color: #02a1e5; max-width:200px; min-width: 200px; min-height: 100px; max-height: 100px; } QToolBar > QToolButton { max-width: 50px;  min-width: 50px; min-height: 70px; max-height: 70px; background-color: #36b6ed; padding-left:0; padding-right:0; margin-left:0; margin-right:0; } QToolBar > QToolButton::first { border-top-left-radius:25px; border-bottom-left-radius:25px; } ");

        threemenu->setToolButtonStyle(Qt::ToolButtonIconOnly);
        threemenu->setIconSize(QSize(40, 40));

        threemenu->addAction(twoLogo);
        threemenu->addAction(threeLogo);
        threemenu->addAction(fourLogo);

        //Alignment menus;
        QHBoxLayout* menulayoutfr = new QHBoxLayout;
        menulayoutfr->addWidget(firstmenu);
        menulayoutfr->addWidget(secondmenu);
        menulayoutfr->addWidget(fourmenu);
        menulayoutfr->addWidget(threemenu);

        menulayoutfr->setSpacing(0);
        menulayoutfr->setMargin(0);

        menulayoutfr->setContentsMargins(0,0,0,0);

        QVBoxLayout* menulayout = new QVBoxLayout;
        menulayout->addLayout(menulayoutfr);
        menulayout->addWidget(walletFrame);

        menulayout->setAlignment(walletFrame, Qt::AlignCenter);
...
void BitcoinGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("Thrill Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Thrill network", "", count));
}
...
{
    QString strTitle = tr("Thrill Core"); // default title
...
void BitcoinGUI::oneLogourl()
{
    QDesktopServices open;
    QString link = "http://thrillproject.cc/";
    open.openUrl(QUrl(link));
}

void BitcoinGUI::twoLogourl()
{
    QDesktopServices open;
    QString link = "https://discord.gg/W9R5CM2";
    open.openUrl(QUrl(link));
}

void BitcoinGUI::threeLogourl()
{
    QDesktopServices open;
    QString link = "https://t.me/thrillproject";
    open.openUrl(QUrl(link));
}

void BitcoinGUI::fourLogourl()
{
    QDesktopServices open;
    QString link = "https://twitter.com/thrillprojectcc";
    open.openUrl(QUrl(link));
}
