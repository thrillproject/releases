...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::THRILL)
    {
    }
    ...
    inline void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
    {
        ...
        QRect amountRect(mainRect.left() + xspace, mainRect.top() + 4, mainRect.width() - xspace - ICON_OFFSET, halfheight);
        QRect addressRect(mainRect.left() - 5 + xspace, mainRect.top() + 10 + halfheight, mainRect.width() - xspace, halfheight);
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - unconfirmedBalance - anonymizedBalance, false, BitcoinUnits::separatorAlways));
    ui->labelBalance->setStyleSheet("color:#00a2e8;");

    ui->labelUnconfirmed->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, unconfirmedBalance, false, BitcoinUnits::separatorAlways));
    ui->labelUnconfirmed->setStyleSheet("color:#00a2e8;");

    ui->labelImmature->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, immatureBalance, false, BitcoinUnits::separatorAlways));
    ui->labelImmature->setStyleSheet("color:#00a2e8;");

    ui->labelAnonymized->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, anonymizedBalance, false, BitcoinUnits::separatorAlways));
    ui->labelAnonymized->setStyleSheet("color:#00a2e8;");
    ...
    ui->labelTotal->setStyleSheet("color:#00a2e8;");
    ...
    bool showImmature = immatureBalance == 0 || immatureBalance != 0;
    //bool showWatchOnlyImmature = watchImmatureBalance == 0 || watchImmatureBalance != 0;
...
void OverviewPage::updateWatchOnlyLabels(bool showWatchOnly)
{
    ...
    ui->labelWatchImmature->setVisible(showWatchOnly);    // show watch-only balance separator line
    ...
    /*
    if (!showWatchOnly) {
        ui->labelWatchImmature->setIndent(20);
    } else {
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("THRILL")
    updateDisplayUnit();
}
...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTHRILLAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTHRILLAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeTHRILLAmount = strAnonymizeTHRILLAmount.remove(strAnonymizeTHRILLAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTHRILLAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeTHRILLAmount * COIN) nMaxToAnonymize = nAnonymizeTHRILLAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeTHRILLAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeTHRILLAmount));
        strAnonymizeTHRILLAmount = strAnonymizeTHRILLAmount.remove(strAnonymizeTHRILLAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTHRILLAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::on_pushButton_5_pressed()
{
  QDesktopServices open;
  QString link = "https://wallet.crypto-bridge.org/market/BRIDGE.THRL_BRIDGE.BTC";
