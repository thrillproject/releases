...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("THRILLMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("THRILLMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("THRILLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("thrill-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("THRILLMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running THRILLMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadTHRILLMiner() exception");
    } catch (...) {
        LogPrintf("ThreadTHRILLMiner() exception");
    }

    LogPrintf("ThreadTHRILLMiner exiting\n");
}
