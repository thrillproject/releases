...
pkgdatadir = $(datadir)/thrill
pkgincludedir = $(includedir)/thrill
pkglibdir = $(libdir)/thrill
pkglibexecdir = $(libexecdir)/thrill
...
DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/Makefile.am \
	...
	$(top_srcdir)/src/config/thrill-config.h.in \
...
CONFIG_HEADER = $(top_builddir)/src/config/thrill-config.h
...
ACLOCAL = ${SHELL} /root/THRILL/THRILL-SRC_CODE/build-aux/missing aclocal-1.14
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/THRILL/THRILL-SRC_CODE/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/THRILL/THRILL-SRC_CODE/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/THRILL/THRILL-SRC_CODE/build-aux/missing automake-1.14
AWK = gawk
BDB_CPPFLAGS = 
...
MAINT = 
MAKEINFO = ${SHELL} /root/THRILL/THRILL-SRC_CODE/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = thrill
PACKAGE_BUGREPORT = http://bit.thrills
PACKAGE_NAME = Thrill Core
PACKAGE_STRING = Thrill Core 1.0.0
PACKAGE_TARNAME = thrill
PACKAGE_URL = 
...
ZMQ_CFLAGS =  
ZMQ_LIBS = -lzmq  
abs_builddir = /root/THRILL/THRILL-SRC_CODE
abs_srcdir = /root/THRILL/THRILL-SRC_CODE
abs_top_builddir = /root/THRILL/THRILL-SRC_CODE
abs_top_srcdir = /root/THRILL/THRILL-SRC_CODE
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/THRILL/THRILL-SRC_CODE/build-aux/install-sh
...
pdfdir = ${docdir}
prefix = /root/THRILL/THRILL-SRC_CODE/depends/i686-pc-linux-gnu
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/thrilld$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/thrill-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/thrill-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Thrill-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_thrill_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_thrill_coverage.info test_thrill.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Thrill-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/thrill-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/thrill-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/thrill-config.h
$(top_srcdir)/src/config/thrill-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/thrill-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Thrill-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Thrill-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Thrill-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_thrill.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_thrill -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_thrill_filtered.info: test_thrill.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_thrill_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_thrill_coverage.info: baseline_filtered_combined.info test_thrill_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_thrill_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_thrill_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_thrill_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_thrill.coverage/.dirstamp:  test_thrill_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_thrill.coverage/.dirstamp total.coverage/.dirstamp

# Thrill: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_thrill.coverage/ total.coverage/ $(OSX_APP)
